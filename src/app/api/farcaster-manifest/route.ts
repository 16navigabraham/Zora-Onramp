...
export async function GET() {
  const manifest = {
    ...
    frame: {
      ...
      webhookUrl: `${process.env.NEXT_PUBLIC_BACKEND_URL || 'https://zora-onramp-backend.onrender.com'}/api/farcaster/webhook`,
    },
    miniApp: {
      name: 'ASAP - Zora Onramp',
      description: 'Fast and easy onramp to USDC on Base for Zora, Base app, and Wallet users',
      icon: `${process.env.NEXT_PUBLIC_APP_URL || 'https://useasap.xyz'}/icon.png`,
      splashImage: `${process.env.NEXT_PUBLIC_APP_URL || 'https://useasap.xyz'}/splash.png`,
